...
//        initializeContacts();
    ...
    public void transactionlist() {
        ...
        viewModel.getLastTwoTransfersIfExists(new BankTransferViewModel.LastTwoCallback() {
            @Override
            public void onResult(LastTwoTransfersWrapper result) {
                requireActivity().runOnUiThread(() -> {
                    // Show latest transaction in transaction2
                    if (result.first != null) {
                        transaction2.setVisibility(View.VISIBLE);
                        user2.setText(result.first.senderName);
                        datetime2.setText((result.first.longdatetime));
                        price2.setText("₦" + result.first.amount);
                    }

                    // If there’s a second recent transaction, show it in transaction1
                    if (result.second != null) {
                        transaction1.setVisibility(View.VISIBLE);
                        user1.setText(result.second.senderName);
                        datetime1.setText((result.second.longdatetime));
                        price1.setText("₦" + result.second.amount);
                    }
                });
            }
    ...
    public void checkAndUpdateIfNewTransfer() {
        ...
        viewModel.checkForNewTransfer(new BankTransferViewModel.NewTransferCallback() {
            @Override
            public void onNewTransfer(BankTransfer newTransfer) {
                requireActivity().runOnUiThread(() -> {
                    // Move transaction2 -> transaction1
                    user1.setText(user2.getText());
                    datetime1.setText(datetime2.getText());
                    price1.setText(price2.getText());
                    transaction1.setVisibility(View.VISIBLE);

                    // Set new transfer to transaction2
                    user2.setText(newTransfer.senderName);
                    datetime2.setText((newTransfer.longdatetime));
                    price2.setText(String.valueOf(newTransfer.amount));
                    transaction2.setVisibility(View.VISIBLE);
                });
            }
